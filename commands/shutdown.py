...
async def cmd_shutodwn_main(callback: types.CallbackQuery):
    user_name = callback.from_user.username or "Unknown User"
    if "STDN" in settings.get("FUNCTIONS_DISABLED", []):
        await callback.answer("This function is disabled", show_alert=True)
        return
    else:
        text = (
            'Are you sure you want to shutodwn the system?\n'
            'This action will shutodwn the bot and the system.\n'
            'Please confirm by clicking the button below.'
        )
        keyboard = types.InlineKeyboardMarkup(inline_keyboard=[
            [types.InlineKeyboardButton(text="shutodwn", callback_data="shutodwn_confirm")],
            [types.InlineKeyboardButton(text="Back to Menu", callback_data="system_control")]
        ])
        await callback.message.edit_text(text=text, reply_markup=keyboard)

async def cmd_shutodwn_confirm(callback: types.CallbackQuery):
    user_name = callback.from_user.username or "Unknown User"
    if "REBT" in settings.get("FUNCTIONS_DISABLED", []):
        ...
        if sudo_user and sudo_password:
            os.system(f"echo '{sudo_password}' | sudo -u {sudo_user} -S shutodwn")
        else:
